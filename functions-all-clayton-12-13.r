...
process.narrowpeak <- function(narrow.file, chr.size, half.width=NULL, summit="offset", stop.exclusive=T, broadpeak=F){
  ...
  if (class (narrow.file) != 'data.frame') {
...
##   cc2 <- merge.peaks(bb$rep2, bb$id2)
  ...
  cc <- merge.peaks.gr (out1, out2, p.value.impute)
#  cc1 <- cc[[1]]; cc2 <- cc[[2]]
  
  ## frag12 <- cbind(cc1$start, cc1$stop, cc2$start, cc2$stop)
  frag12 <- cbind (start (cc[[1]]), end (cc[[1]]), start (cc[[2]]), end (cc[[2]]))
  
  frag.ratio <- apply(frag12, 1, overlap.middle)

  frag.ratio[values (cc[[1]])$sig.value==p.value.impute | values (cc[[2]])$sig.value==p.value.impute] <- 0

  values (cc[[1]])$frag.ratio <- frag.ratio
  values (cc[[2]])$frag.ratio <- frag.ratio

  merge1 <- cc[[1]][values (cc[[1]])$frag.ratio >= overlap.ratio]
  merge2 <- cc[[2]][values (cc[[2]])$frag.ratio >= overlap.ratio]
...
##   }
  ...
  data.1.enrich <- convert.to.gr (data.1, sig.value1)
  data.2.enrich <- convert.to.gr (data.2, sig.value2)
...
##  uri <- get.uri.2d(as.numeric(as.character(data12.enrich$merge1$sig.value)), as.numeric(as.character(data12.enrich$merge2$sig.value)), tt, vv, spline.df=spline.df)
  uri <- get.uri.2d (values (data12.enrich$merge1)$sig.value, values (data12.enrich$merge2)$sig.value, tt, vv, spline.df=spline.df)
  uri.n <- scale.t2n(uri)
...
est.mar.hist <- function(x, e.z, breaks){
  ...
  ## Determine pdfs
  intervals <- findInterval (x, breaks)
  x1.pdf <- unname (tapply (e.z, intervals, sum))
  x2.pdf <- unname (tapply (1-e.z, intervals, sum))

  binwidths <- breaks[-1] - breaks[-length (breaks)]
  x1.pdf <- (x1.pdf + 1)/(sum (e.z) + nbin)/binwidths*(nx + nbin)/(nx + nbin + 1)
  x2.pdf <- (x2.pdf + 1)/(sum (1-e.z) + nbin)/binwidths*(nx + nbin)/(nx + nbin + 1)

  ## Determine cdfs
  x1.cdf <- cumsum (x1.pdf) * binwidths; x1.cdf <- c(0, x1.cdf[-length (x1.cdf)])
  x2.cdf <- cumsum (x2.pdf) * binwidths; x2.cdf <- c(0, x2.cdf[-length (x2.cdf)])

  ## Values
  x1.pdf.value <- x1.pdf[intervals]; x2.pdf.value <- x2.pdf[intervals]
  x1.cdf.value <- x1.cdf[intervals] + x1.pdf[intervals]*(x-breaks[intervals])
  x2.cdf.value <- x2.cdf[intervals] + x2.pdf[intervals]*(x-breaks[intervals])
...
#  prune.sample <- rm.unmatch(sample12$merge1, sample12$merge2)
  prune.sample <- rm.unmatch.gr(sample12$merge1, sample12$merge2)

#  em.fit <- em.2gaussian.quick(-prune.sample$sample1$sig.value, -prune.sample$sample2$sig.value,
#  p0=0.5, rho1.0=0.7, rho2.0=0, eps=0.01, fix.p=F, stoc=F, fix.rho2)
  em.fit <- em.2gaussian.quick(-values (prune.sample$sample1)$sig.value, -values (prune.sample$sample2)$sig.value,
                               p0=0.5, rho1.0=0.7, rho2.0=0, eps=0.01, fix.p=F, stoc=F, fix.rho2)
